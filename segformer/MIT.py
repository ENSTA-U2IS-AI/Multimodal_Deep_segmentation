...
class SegFormerHead(nn.Module):
    ...
    """
    def __init__(self, in_channels,feature_strides,decoder_params,num_classes,dropout_ratio=0.1):
        super().__init__()
        ...
        self.linear_c4 = MLP_head(input_dim=c4_in_channels, embed_dim=embedding_dim)
        self.linear_c3 = MLP_head(input_dim=c3_in_channels, embed_dim=embedding_dim)
        self.linear_c2 = MLP_head(input_dim=c2_in_channels, embed_dim=embedding_dim)
        self.linear_c1 = MLP_head(input_dim=c1_in_channels, embed_dim=embedding_dim)
...
if __name__=='__main__':
    ...
    model = SegFormer()
    output = model(x)
    print(output.size())  # torch.Size([1, 100, 56, 56])
    print(dict(type='Resize', keep_ratio=True))
