...
def GMM_fit_v1(model,loader,device):
    with torch.no_grad():
        ...
        gmm = GaussianMixture(n_components=17, random_state=0).fit(embeddings_sample.detach().cpu().numpy())
...
def validate(opts, model, loader,loader_train, device, metrics, ret_samples_ids=None):
    ...
    if opts.save_val_results:
        ...
        denorm = utils.Denormalize(mean=[0.485, 0.456, 0.406], 
                                   std=[0.229, 0.224, 0.225])
        img_id = 0
    gmm = GMM_fit_v1(model, loader_train, device)
    with torch.no_grad():
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            outputs_feature=outputs.clone()
            outputs_feature_tmp = rearrange(outputs_feature, 'b h n d -> b n d h')
            print(outputs_feature_tmp.size())
            out0=torch.reshape(outputs_feature_tmp[0] ,(1024*512,19))
            out1 = torch.reshape(outputs_feature_tmp[1], (1024 * 512, 19))
            conf_v2_0 =gmm.score_samples(out0.cpu().detach().numpy())
            conf_v2_1  = gmm.score_samples(out1.cpu().detach().numpy())
            conf_v2_0= torch.from_numpy(conf_v2_0).cuda()
            conf_v2_0 = torch.reshape(conf_v2_0, (1, 512,1024))
            conf_v2_1= torch.from_numpy(conf_v2_1).cuda()
            conf_v2_1 = torch.reshape(conf_v2_1, (1, 512,1024))
