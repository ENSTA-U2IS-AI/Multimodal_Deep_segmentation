...
from .utils import _SimpleSegmentationModel, _SimpleSegmentationModel_DM,_SimpleSegmentationModel_DM2,\
    _SimpleSegmentationModel_DM3,_SimpleSegmentationModel_DM4
...
class DeepLabV3DM_v3(_SimpleSegmentationModel_DM3):
    ...
    """
    pass
class DeepLabV3DM_v4(_SimpleSegmentationModel_DM4):
    """
    Implements DeepLabV3 model from
    `"Rethinking Atrous Convolution for Semantic Image Segmentation"
    <https://arxiv.org/abs/1706.05587>`_.
...
class DeepLabHeadV3Plus_DM_v2_xuanlong(nn.Module):
    def __init__(self, in_channels, low_level_channels, num_classes, aspp_dilate=[12, 24, 36]):
        super(DeepLabHeadV3Plus_DM_v2_xuanlong, self).__init__()
        self.project = nn.Sequential(
            nn.Conv2d(low_level_channels, 48, 1, bias=False),
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True),
        ...
        self.DMlayer = Distanceminimi_Layer_learned_old(in_features=256, out_features=num_classes, dist='cos')
        #self.conv1x1 = nn.Conv2d(256, self.nbproto, 1)
        self.bn1 = nn.BatchNorm2d(num_classes)
        self.bn2 = nn.BatchNorm2d(256)
        self.conv1x1 = nn.Conv2d( num_classes,256, 1)
        self.lastlayer = nn.Conv2d(256, num_classes, 1)
        self._init_weight()
    ...
    def forward(self, feature):
        ...
        out = rearrange(embedding, 'b n d h -> b h n d')
        out =self.bn1(out)
...
class DeepLabHeadV3v5Plus_DM(nn.Module):
    def __init__(self, in_channels, low_level_channels, num_classes, aspp_dilate=[12, 24, 36]):
        super(DeepLabHeadV3v5Plus_DM, self).__init__()
        self.project = nn.Sequential(
            nn.Conv2d(low_level_channels, 48, 1, bias=False),
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True),
...
class DeepLabHeadV3v4Plus_DM(nn.Module):
    def __init__(self, in_channels, low_level_channels, num_classes, aspp_dilate=[12, 24, 36]):
        super(DeepLabHeadV3v4Plus_DM, self).__init__()
        self.project = nn.Sequential(
            nn.Conv2d(low_level_channels, 48, 1, bias=False),
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True),
...
class DeepLabHeadV3v2Plus_DM(nn.Module):
    def __init__(self, in_channels, low_level_channels, num_classes, aspp_dilate=[12, 24, 36]):
        super(DeepLabHeadV3v2Plus_DM, self).__init__()
        self.project = nn.Sequential(
            nn.Conv2d(low_level_channels, 48, 1, bias=False),
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True),
