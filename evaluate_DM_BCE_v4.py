...
def get_argparser():
    ...
    parser.add_argument("--model", type=str, default='deeplabv3plus_mobilenet',
                        choices=['deeplabv3_resnet50',  'deeplabv3plus_resnet50','deeplabv3plus_resnet50_DM','deeplabv3plus_resnet50_drop','deeplabv3plus_resnet50_DMv3v2',
                                 'deeplabv3_resnet101', 'deeplabv3plus_resnet101','deeplabv3plus_resnet101_DM','deeplabv3plus_resnet50_DMv4','deeplabv3plus_resnet50_DMv3v5',
                                 'deeplabv3_mobilenet', 'deeplabv3plus_mobilenet','FCN_resnet50','deeplabv3plus_resnet50_DMv3v3','deeplabv3plus_resnet50_DMv3v4',
                                 'deeplabv3plus_resnet50_DMv3v6','deeplabv3plus_spectral50','deeplabv3plus_resnet50_DMv2'], help='model name')
...
def eval_ood_measure(conf, seg_label,name, mask=None):
    ...
    out_labels = [16,17,18]# [16,17,18,255]
    if mask is not None:
...
def validate(opts, model, loader, device, metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            labels =torch.clamp(labels, min=0, max=18)
            ...
            if  (opts.phase=='test_OOD') or (opts.phase=='test_level1') or (opts.phase=='test_level2'):
                try:
                    conf2=conf[labels!=255]
                    labels2=labels[labels!=255]
                    auroc, aupr, fpr = eval_ood_measure(conf2, labels2,name_img, mask=mask)
                except:
            ...
            #conf = conf/conf.max()
            ###print(name_img0)
            '''name_img02=name_img0+str(i)+'_MCP.jpg'
            img_conf=((conf[0]* 255).detach().cpu().numpy()).astype(np.uint8)
            print('name_img0',name_img0,np.shape(img_conf),img_conf)
            Image.fromarray(img_conf).save('results/new_BCE_ECCV/'+name_img02)
            print('np.unique(img_conf)',np.unique(img_conf))'''
...
def create_ema_model(model,modelname,num_classes,output_stride,gpus):
    model_map = {
        ...
        'deeplabv3plus_resnet50_DMv3v6': network.deeplabv3plus_resnet50_DM_v3v6,
