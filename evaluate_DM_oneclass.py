...
def validate(opts, model, loader_train, loader, device, metrics, ret_samples_ids=None,nu=0.5,gamma=0.5):
    ...
    with torch.no_grad():
        for i, (images, labels,name_img) in tqdm(enumerate(loader_train)):
            ...
            if i == 0: embeddings_train=outputs_feature_train.clone()
            else : embeddings_train = torch.cat((embeddings_train, outputs_feature_train), 0)
...
def main():
    ...
    for gamma_i in gammalist:
      for nu0 in nulist:#for gamma_i in gammalist:
          nu=float(nu0)/100.0
          val_score, ret_samples,ece,NLL, auroc_list, aupr_list, fpr_list  = validate(opts=opts, model=model, loader_train=train_loader, loader=val_loader, device=device, metrics=metrics,nu=nu,gamma=gamma_i, ret_samples_ids=vis_sample_id)
