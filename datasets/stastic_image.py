    ...
    **Parameters:**
    ...
    def __getitem__(self, index):
        ...
        Args:
            index (int): Index
        Returns:
            tuple: (num_image, num_pixel) where num_image is the number of images including each class, num_pixel is the
            number if pixels including each class.
        """
        ...
        num_image = np.zeros(155)
        num_pixel = np.zeros(155)

        for c in self.classes:
            upper = np.array(c.object_id)
            lower = upper
            mask = cv.inRange(segm, lower, upper)
            size = mask[mask==255].size
            if size != 0:
                num_image[c.id] += 1
                num_pixel[c.id] += size
        return num_image, num_pixel
...
if __name__ == '__main__':
    ...
    #----- calculate the stastics of images -----#
    num_images = torch.zeros(155)
    num_pixels = torch.zeros(155)
    train_data = dataset(root_dataset='../av-challenge', root_odgt='../av-challenge/data_odgt/train.odgt')
    train_queue = torch.utils.data.DataLoader(
        train_data, batch_size=2, pin_memory=True, num_workers=16, shuffle=True)
    for step, (num_image, num_pixel) in enumerate(train_queue):
        num_images = num_images + num_image.sum(dim=0)
        num_pixels = num_pixels + num_pixel.sum(dim=0)

    #----- draw the histogram -----#
    plt.figure(figsize=(20,10))
    plt.suptitle('stastics of dataset', fontsize=30)
    ax1= plt.subplot(211)
    plt.bar(range(0,155), num_images.numpy().flatten())
    plt.xlabel('classes', fontsize=15)
    plt.ylabel('number of images',fontsize=15)
    plt.axis(xmin=0, xmax=154)
    plt.tick_params(axis='both', which='major', labelsize=6)
    x_major_locator = MultipleLocator(2)
    ax1.xaxis.set_major_locator(x_major_locator)

    ax2 = plt.subplot(212)
    plt.bar(range(0, 155), num_pixels.numpy().flatten())
    plt.xlabel('classes', fontsize=15)
    ...
    #----- save the data -----#
    h5f = h5py.File(f'./data/stastic_dataset.hdf5', 'w')
    h5f.create_dataset('num_image', data=num_images.numpy().flatten())
    h5f.create_dataset('num_pixel', data=num_pixels.numpy().flatten())
    h5f.close()
    print(f'num_image: {num_images.numpy().flatten()}', '\n', f'num_pixel: {num_pixels.numpy().flatten()}')