...
def validate(opts, model0,model1,model2, loader, device, metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            outputsproba=(1/(3.0))*(Softmax(outputs0)+Softmax(outputs1)+Softmax(outputs2))
            outputsproba=outputsproba.detach().type(torch.float64)
            preds = outputsproba.max(dim=1)[1].cpu().numpy()
            outputslogproba=torch.log(outputsproba)

            nll_out=criterion_NLL(outputslogproba, labels)
            #nll_out=criterion_CE(outputsproba, labels)
            #print(nll_out)
...
def main():
    ...
    model0 = model_map[opts.model](num_classes=opts.num_classes, output_stride=opts.output_stride)
    ...
    if opts.separable_conv and 'plus' in opts.model:
        network.convert_to_separable_conv(model0.classifier)
        network.convert_to_separable_conv(model1.classifier)
        network.convert_to_separable_conv(model2.classifier)
    utils.set_bn_momentum(model0.backbone, momentum=0.01)
    ...
    optimizer = torch.optim.SGD(params=[
        {'params': model0.backbone.parameters(), 'lr': 0.1*opts.lr},
        {'params': model0.classifier.parameters(), 'lr': opts.lr},
    ...
    if opts.ckpt is not None and os.path.isfile(opts.ckpt):
        if opts.dataset.lower() == 'av':
            # https://github.com/VainF/DeepLabV3Plus-Pytorch/issues/8#issuecomment-605601402, @PytaichukBohdan
            checkpoint = torch.load('/home/soumik/workspace_hard1/workspacegianni/Deeplabuncertaintysurvey/checkpoints/DE0/best_deeplabv3plus_resnet50_av_os8.pth', map_location=torch.device('cpu'))
            model0.load_state_dict(checkpoint["model_state"])
            model0 = nn.DataParallel(model0)
            model0.to(device)
            checkpoint = torch.load('/home/soumik/workspace_hard1/workspacegianni/Deeplabuncertaintysurvey/checkpoints/DE1/best_deeplabv3plus_resnet50_av_os8.pth', map_location=torch.device('cpu'))
            model1.load_state_dict(checkpoint["model_state"])
            model1 = nn.DataParallel(model1)
            model1.to(device)
            checkpoint = torch.load('/home/soumik/workspace_hard1/workspacegianni/Deeplabuncertaintysurvey/checkpoints/DE2/best_deeplabv3plus_resnet50_av_os8.pth', map_location=torch.device('cpu'))
            model2.load_state_dict(checkpoint["model_state"])
            model2 = nn.DataParallel(model2)
            model2.to(device)
        elif (opts.dataset.lower() == 'cityscapes')or(opts.dataset == 'cityscapes_C'):
            # https://github.com/VainF/DeepLabV3Plus-Pytorch/issues/8#issuecomment-605601402, @PytaichukBohdan
            checkpoint = torch.load('/home/soumik/workspace_hard1/workspacegianni/Deeplabuncertaintysurvey/checkpoints/DE0_city/best_deeplabv3plus_resnet50_cityscapes_os8.pth', map_location=torch.device('cpu'))
            model0.load_state_dict(checkpoint["model_state"])
            model0 = nn.DataParallel(model0)
            model0.to(device)
            checkpoint = torch.load('/home/soumik/workspace_hard1/workspacegianni/Deeplabuncertaintysurvey/checkpoints/DE1_city/best_deeplabv3plus_resnet50_cityscapes_os8.pth', map_location=torch.device('cpu'))
            model1.load_state_dict(checkpoint["model_state"])
            model1 = nn.DataParallel(model1)
            model1.to(device)
            checkpoint = torch.load('/home/soumik/workspace_hard1/workspacegianni/Deeplabuncertaintysurvey/checkpoints/DE2_city/best_deeplabv3plus_resnet50_cityscapes_os8.pth', map_location=torch.device('cpu'))
            model2.load_state_dict(checkpoint["model_state"])
            model2 = nn.DataParallel(model2)
            model2.to(device)
    ...
    val_score, ret_samples,ece,NLL, auroc_list, aupr_list, fpr_list  = validate(
        opts=opts, model0=model0, model1=model1, model2=model2, loader=val_loader, device=device, metrics=metrics, ret_samples_ids=vis_sample_id)
    NLLarray=np.asarray(NLL)
    print(metrics.to_str(val_score))
    print('--------------------------------------------------------------------')
    print('ECE!!!!! mean ECE = ', np.mean(np.asarray(ece)))
    print('NLL!!!!! mean NLL = ', np.mean(NLLarray[NLLarray!=np.inf]))
